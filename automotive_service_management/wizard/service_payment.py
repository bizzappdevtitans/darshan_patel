...
class ServicePayment(models.TransientModel):
    ...
    appointment_id = fields.Many2one(comodel_name="service.appointment", string="Appointment")
    serviced_amount = fields.Integer(string="Total Amount")
    payment_mode = fields.Selection([('upi', 'UPI'), ('bank_account', 'Bank Account')],
                                    default="upi")
    ...
    payment_reference_number = fields.Char(string="Payment Reference Number", required=True)

    @api.onchange('appointment_id')
    def get_amount(self):
        for record in self:
            record.update({'serviced_amount': self.appointment_id.total_cost})
            record.update({'upi_number': self.env['ir.config_parameter'].get_param(
                           "automotive_service_management.upi_number")})
            record.update({'ifsc_code': self.env['ir.config_parameter'].get_param(
                           "automotive_service_management.ifsc_code")})
            record.update({'account_number': self.env['ir.config_parameter'].get_param(
    ...
    def action_payment_done(self):
        for record in self.appointment_id:
            if record.state == "serviced":
                ...
                record.write({"payment_reference_number": self.payment_reference_number + " - " +self.payment_mode})
            return {
                'effect': {
                    'fadeout': 'slow',
