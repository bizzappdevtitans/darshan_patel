...
class SaleOrderSplitQuotation(models.TransientModel):
    ...
    # action_split_sale_order for split sale order based on split sale order #T00482
    def action_split_sale_order(self):
        """this method is use for split sale order #T00482"""
        ...
        # sale order based on category #T00482
        if self.split_sale_order == "based_on_category":
            # this method split sale order based on category #T00482
        ...
        # selected product #T00482
        elif self.split_sale_order == "selected_lines":
            # this method create sale order on selected product #T00482
            if not sale_order_id.sale_order_reference:
        ...
        # sale order for each product in sale order #T00482
        elif self.split_sale_order == "one_line_per_order":
            # this function will execute only if split_sale_order is
            # one line per order #T00482
            if not sale_order_id.sale_order_reference:
                for product in sale_order_id.order_line:
                    self.env["sale.order"].create(
                        {
